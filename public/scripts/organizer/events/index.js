$(function () {
    ...
    const calendar = new window.Fullcalendar.Calendar($("#calendar").get(0), {
        ...
        eventClick: (info) => {
            ...
            window.location.href = config.routes.show.replace(
                "resource_id",
                event.code
            );
        },
        dateClick: (info) => {
            ...
            $(".add-event-button").attr(
                "href",
                config.routes.create + `/?start=${start}&end=${end}`
            );
                    ...
                    if (Object.keys(eventsOfTheDay).length) {
                        events = $.map(
                            ...
                            function (events, index) {
                                return `<h3>${moment(index).format(
                                    "MMMM D YYYY"
                                )}</h3> <div class="col-md-12"> ${constructEventList(
                                    events,
                                    index
                                )}</div><hr>`;
                            }
    ...
    function dateModal(info, events, add_buton_bool = true) {
        ...
        let event_for = info.dateStr
            ...
            : `${readableFormat(info.startStr)} - ${readableFormat(
                  moment(info.endStr).subtract(1, "days")
              )}`;
        ...
        modals.date
            .find(".toggle-concluded-events")
            .off()
            .on("click", (_) => {
                modals.date.find(".event").each((i, event_div) => {
                    let concludedEvent =
                        $(event_div).data("status") == "CONCLUDED";
        ...
        modals.date.find(".event-status").each((i, status_div) => {
            ...
            intervals[i] = setInterval((_) => {
                ...
                switch (status_data.status) {
                    ...
                    case "ONGOING":
                        let timeDiffEnd = moment.duration(
                            moment(schedule_end).diff(moment())
                        ); //! for ongoing to concluded
                        let secondsLeftEnd = parseFloat(
                            timeDiffEnd.format("s").replace(/,/g, "")
                        );

                        if (secondsLeftEnd < 0) {
                            status_html =
                                '<b class="text-muted">CONCLUDED</b >';
                    ...
                    default: //*SOON
                        let timeDiff = moment.duration(
                            moment(schedule_start).diff(moment())
                        );
                        let countdown = timeDiff.format("hh : mm : ss");
                        let secondsLeft = parseFloat(
                            timeDiff.format("s").replace(/,/g, "")
                        );
        ...
        eventCount
            ? modals.date.find(".toggle-concluded-events").show()
            : modals.date.find(".toggle-concluded-events").hide();
    ...
    function constructEventList(schedules, day) {
        let event_row = (schedule) => {
            ...
            */
            let edit_button = moment(schedule.schedule_start).isBefore()
                ? ""
                : `<a class=" btn btn-link btn-light" href="${config.routes.edit.replace(
                      "resource_id",
                      schedule.event.code
                  )}">Edit</a>`;
            ...
            <div class="scheduled_time">
                <strong>${
                    date_formater(schedule.schedule_start).time
                }</strong> - <strong>${
                date_formater(schedule.schedule_end).time
            }</strong>
            </div>

            <div class="event-status" data-status="${
                schedule.status
            }" data-day="${day}" data-start="${
                schedule.schedule_start
            }" data-end="${schedule.schedule_end}"></div>

            <a class=" btn btn-link btn-light" href="${config.routes.show.replace(
                "resource_id",
                schedule.event.code
            )}">Preview</a>

            <a class=" btn btn-link btn-light" href="${config.routes.invitations.replace(
                "resource_id",
                schedule.event.code
            )}">Attendees</a>
    ...
    function fetchScheduleEvents(date, callback) {
        axios
            .get(config.routes.fetchScheduleEvents, { params: date })
